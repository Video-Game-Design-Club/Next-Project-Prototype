...
{
    [Serializable]
    internal class ScreenSpaceAmbientOcclusionSettingsNYAA
    {
        // Parameters
    ...
    [DisallowMultipleRendererFeature("Screen Space Ambient Occlusion NYAA")]
    [Tooltip("The Ambient Occlusion effect darkens creases, holes, intersections and surfaces that are close to each other.")]
    //[URPHelpURL("post-processing-ssao")]
    internal class ScreenSpaceAmbientOcclusionNYAA : ScriptableRendererFeature
    {
        // Serialized Fields
        [SerializeField] private ScreenSpaceAmbientOcclusionSettingsNYAA m_Settings = new ScreenSpaceAmbientOcclusionSettingsNYAA();
        ...
        private ScreenSpaceAmbientOcclusionNYAAPass m_SSAOPass = null;

        // Internal / Constants
        internal ref ScreenSpaceAmbientOcclusionSettingsNYAA settings => ref m_Settings;
        ...
        public override void Create()
        {
            ...
            // Create the pass...
            if (m_SSAOPass == null)
                m_SSAOPass = new ScreenSpaceAmbientOcclusionNYAAPass();

            // Check for previous version of SSAO
            if (m_Settings.SampleCount > 0)
            {
                m_Settings.AOMethod = ScreenSpaceAmbientOcclusionSettingsNYAA.AOMethodOptions.InterleavedGradient;

                if (m_Settings.SampleCount > 11)
                    m_Settings.Samples = ScreenSpaceAmbientOcclusionSettingsNYAA.AOSampleOption.High;
                else if (m_Settings.SampleCount > 8)
                    m_Settings.Samples = ScreenSpaceAmbientOcclusionSettingsNYAA.AOSampleOption.Medium;
                else
                    m_Settings.Samples = ScreenSpaceAmbientOcclusionSettingsNYAA.AOSampleOption.Low;
        ...
        // The SSAO Pass
        internal class ScreenSpaceAmbientOcclusionNYAAPass : ScriptableRenderPass
        {
            // Properties
            private bool isRendererDeferred = false;//=> m_Renderer != null && m_Renderer is UniversalRenderer && ((UniversalRenderer)m_Renderer).renderingModeRequested == RenderingMode.Deferred;
            ...
            private ProfilingSampler m_ProfilingSampler = new ProfilingSampler("NYAAAAA");//ProfilingSampler.Get(URPProfileId.SSAO);
            private ScriptableRenderer m_Renderer = null;
            private RenderTextureDescriptor m_AOPassDescriptor;
            private ScreenSpaceAmbientOcclusionSettingsNYAA m_CurrentSettings;
            ...
            internal ScreenSpaceAmbientOcclusionNYAAPass()
            {
                m_CurrentSettings = new ScreenSpaceAmbientOcclusionSettingsNYAA();
            }

            internal bool Setup(ref ScreenSpaceAmbientOcclusionSettingsNYAA featureSettings, ref ScriptableRenderer renderer, ref Material material, ref Texture2D[] blueNoiseTextures)
            {
                ...
                if (isRendererDeferred)
                {
                    ...
                    m_CurrentSettings.Source = ScreenSpaceAmbientOcclusionSettingsNYAA.DepthSource.DepthNormals;
                }
                else
                {
                ...
                {
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.DepthSource.Depth:
                        ConfigureInput(ScriptableRenderPassInput.Depth);
                        break;
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.DepthSource.DepthNormals:
                ...
                {
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.BlurQualityOptions.High:
                        m_BlurType = BlurTypes.Bilateral;
                        break;
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.BlurQualityOptions.Medium:
                        m_BlurType = BlurTypes.Gaussian;
                        break;
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.BlurQualityOptions.Low:
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                {
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.AOMethodOptions.BlueNoise:
                        ...
                        break;
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.AOMethodOptions.InterleavedGradient:
                ...
                {
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.AOSampleOption.High:
                        CoreUtils.SetKeyword(m_Material, k_SampleCountHighKeyword, true);
                        break;
                    case ScreenSpaceAmbientOcclusionSettingsNYAA.AOSampleOption.Medium:
                ...
                if (m_CurrentSettings.Source == ScreenSpaceAmbientOcclusionSettingsNYAA.DepthSource.Depth)
                {
                    CoreUtils.SetKeyword(m_Material, k_SourceDepthNormalsKeyword, false);
                    switch (m_CurrentSettings.NormalSamples)
                    {
                        case ScreenSpaceAmbientOcclusionSettingsNYAA.NormalQuality.Low:
                            ...
                            break;
                        case ScreenSpaceAmbientOcclusionSettingsNYAA.NormalQuality.Medium:
                            ...
                            CoreUtils.SetKeyword(m_Material, k_SourceDepthHighKeyword, false);
                            break;
                        case ScreenSpaceAmbientOcclusionSettingsNYAA.NormalQuality.High:
                            CoreUtils.SetKeyword(m_Material, k_SourceDepthLowKeyword, false);
                            CoreUtils.SetKeyword(m_Material, k_SourceDepthMediumKeyword, false);
